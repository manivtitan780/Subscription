...
public partial class ViewWordDocument
{
	...
	private async Task OpenDialog()
	{
		...
		MemoryStream _stream = new(await File.ReadAllBytesAsync(Path.Combine("Start.UploadsPath", "Uploads", _entity, EntityID.ToString(), InternalFileName))); //TODO: Include the correct path
		...
		SerializedString = JsonConvert.SerializeObject(_document);
	...
	/// </remarks>
	private void Opening(Syncfusion.Blazor.Popups.BeforeOpenEventArgs obj) => IsOpening = true;
	...
	/// </explain>
	private async Task ToolbarClicked(Syncfusion.Blazor.DocumentEditor.ClickEventArgs args)
	{
		switch (args.Item.Id)
		{
			case "save":
